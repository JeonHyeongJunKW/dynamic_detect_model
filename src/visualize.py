    ...
    with torch.no_grad():
        for idx , (query_images, compare_images, labels) in enumerate(validset):
            if idx in query_idxs:
                ...
                spatial_image = tensor2image(spatial_query[0])
                query_image = tensor2image(query_images[0])
                createFolder(folder)
                createFolder(folder+"/"+sub_folder)
                cv2.imwrite(folder+"/"+sub_folder+"/"+idx+"_query.png",query_image)
                cv2.imwrite(folder+"/"+sub_folder+"/"+idx+"_spatial.png",spatial_image)
